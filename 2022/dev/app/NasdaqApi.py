...
class Nasdaq():
	...
	def GetNasdaqEventsHandler(self, sock, packet):
		...
		print("({classname})# [GetNasdaqEventsHandler] {0}".format(payload,classname=self.ClassName),5)
		data = None
		try:
			event = payload["event"]
			...
			print("({classname})# [EXCEPTION] GetNasdaqEventsHandler {0} {1}".format(payload,str(e),classname=self.ClassName), 5)
